...
Omega = linspace(13.7971410359, 14.3171410359, 100);
...
legend(sprintf('%g', Omega(1)), sprintf('%g', Omega(2)), sprintf('%g', Omega(3)), sprintf('%g', Omega(4)), sprintf('%g', Omega(5)), ...
    sprintf('%g', Omega(6)), sprintf('%g', Omega(7)), sprintf('%g', Omega(8)), sprintf('%g', Omega(9)), sprintf('%g', Omega(10)));

figure
plot(Omega, max_Emec, 'LineWidth',lw)
set(gca,'fontsize',fs)
xlabel('$\Omega$ [rad/s]')
ylabel('$E_{mec}^{max}$ [J]')
grid on
...
for i = 1:nsimul % Parcours des resultats de toutes les simulations
    ...
    tv=data(:,1);
    thetav=data(:,2);
    %plot(tv,thetav,'Color', [i/100 0.4470 0.7410], 'LineWidth',lw)
...
for i = 1:nsimul % Parcours des resultats de toutes les simulations
    ...
    tv=data(:,1);
    thetadotv    = data(:,3);
    %plot(tv,thetadotv,'Color', [i/100 0.4470 0.7410], 'LineWidth',lw)
...
for i = 1:nsimul % Parcours des resultats de toutes les simulations
    ...
    thetav=data(:,2);
    thetadotv    = data(:,3);
    %plot(thetav,thetadotv,'Color', [i/100 0.4470 0.7410], 'LineWidth',lw)
    plot(thetav,thetadotv,'k', 'LineWidth',lw)
    hold on
end
set(gca,'fontsize',fs)
xlabel('$\theta$ [rad]')
