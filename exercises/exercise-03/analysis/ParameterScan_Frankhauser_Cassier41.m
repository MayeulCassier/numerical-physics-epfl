...
theta0=[7*pi/8 7*pi/8+1.0e-8 0 1.0e-8];
...
for j= 1:nthetadot    
    for i = 1:ntheta-1 % Parcours des resultats de toutes les simulations
        ...
        if i==1
            semilogy(t, d, 'k','linewidth',lw);
            r=18000;
            p=polyfit(t(1:r, 1),log(d(1:r, 1)),1);
            f=polyval(p,t(1:r, 1));
            hold on
            semilogy(t(1:r, 1),exp(f), '--', 'linewidth', lw);
            hold off
            if p(2)>0
        ...
        elseif i==3
            ...
            legend("$\dot{\theta}_0=$ "+thetadot0(j)+" et $\theta_0=0$ (ou $10^{-8}$)", fit);
        else 
