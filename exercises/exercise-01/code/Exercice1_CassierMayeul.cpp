...
private:
  ...
  void printOut(bool write)
  {
    ...
    {
      *outputFile << t << " " << mF << " " << x << " " << v \
       << " " << energy(mF,x,v) << " " << power(mF,x,v) << " " << posFinale() << " " << vitFinale() <<endl;
      last = 1;
    }
    else
    {
  ...
  double Fvisc(double xLocal,double vLocal){
    double Fa(0.0);
    Fa = 0.5*rho_0*exp(-xLocal/lambda)*Cx*Surf*vLocal*vLocal;
    return Fa;
  }

  double posFinale()
  {
  	double xfin = -0.5*tfin*(g0*tfin-2.0*v_e)-(v_e/D_f)*(D_f*tfin-mF)*log((-tfin*D_f/mF)+1); 
  	return xfin;
  }
  ...
  double vitFinalebis()
  {
  	double vfin =-g0*tfin + v_e*log(((-D_f*tfin)/mF) +1);
  	return vfin;
  ...
  double acceleration(double mLocal, double xLocal,double vLocal){
    double a(0.0);
    a= (Fprop()-Fvisc(xLocal,vLocal))*(1.0/mLocal)-g0;
    return a;
  }
  ...
  double energy(double mLocal, double xLocal, double vLocal){
    double kinetic(0.5*mLocal*vLocal*vLocal);
    double potential(mLocal*g0*xLocal);
  ...
  double power(double mLocal, double xLocal, double vLocal){
    double p_friction(-vLocal*Fvisc(xLocal,vLocal));
    double p_propulsion(Fprop()*vLocal);
    double p_massvariation(-0.5*D_f*vLocal*vLocal - D_f*g0*xLocal);
  ...
  void step()
  {
    ...
    mF += -D_f*dt;   // mettre a jour la mass
    x  += v*dt ;   // mettre a jour la position
