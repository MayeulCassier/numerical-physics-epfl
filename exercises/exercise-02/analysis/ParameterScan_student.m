...
tfin = 8.2e-08; % TODO: Verifier que la valeur de tfin est EXACTEMENT la meme que dans le fichier input

dt = tfin / nstep1;
paramstr = 'schema'; % Nom du parametre a scanner  MODIFIER SELON VOS BESOINS
param = schema;  % Valeurs du parametre a scanner  MODIFIER SELON VOS BESOINS
eth = 2.09075e-016;

lw =1.5; fs=15;
output = cell(nstep1, nschema); % Tableau de cellules contenant les noms des fichiers de sortie
for i = 1:nschema
    output{i} = [paramstr, '=', num2str(param(i)), '.out'];
    % Execution du programme en lui envoyant la valeur a scanner en argument
...
Emax=zeros(nstep1+1,nschema);
Emax1=zeros(nstep1,nschema);
for i = 1:nschema % Parcours des resultats de toutes les simulations
    data  = load(output{i}); % Chargement du fichier de sortie de la i-ieme simulation
    t     = data(:,1); 
    Emax(:,i)= data(:, 9);
    %mu    = data(:,10);
    for j=1:(nstep1-1)
        Emax1(j,i)=(data(j,9)-data(j+1,9))/dt;
    end
    hold on
end
figure
for i = 1:nschema
    plot(t,Emax(:, i),'linewidth',lw);
    hold on
end
xlabel('$t$ [s]')
ylabel('$E_{mec}$ [J]')
legend("Euler explicite","Euler Implicite", ...
    "Euler Semi-Implicite", "Runge-Kutta2")
set(gca,'fontsize',fs)
grid on
hold off
...
figure
for i = 1:nschema
    plot(t(2:end,1),Emax1(:, i),'linewidth',lw);
    hold on
end
xlabel('$t$ [s]')
...
% norder=1; % TODO: modifier si besoin est
% 
% figure
% plot(dt.^norder,vy,'k+-','linewidth',lw)
% set(gca,'fontsize',fs)
% xlabel('\Delta t [s]')
% ylabel('v_y [m/s]')
% grid on
