...
nsteps = [800 1600 3200 6400 12800];
...
% TODO:  inserer ici les expressions de la solution exacte
    xnorm(i)=sqrt(x(end)^2+y(end)^2+z(end)^2);
...
lw=1.5; fs=16;
figure
p=polyfit(dt.*dt,xnorm,1);
f=polyval(p,dt.*dt);
plot(dt.*dt, xnorm,'black+', 'LineWidth', lw)
hold on
plot(dt.*dt,f, '--', 'linewidth', lw)
set(gca,'fontsize',fs)
xlabel('$\Delta$t$^2$ [s]')
ylabel('$|\vec{x}(t_{fin})|$ [m]')
if p(2)>0
   fit = sprintf('fit:$y=$%0.5g$x+$%0.5g',p(1),p(2));
else 
   fit = sprintf('fit:$y=$%0.5g$x$%0.5g',p(1),p(2));
end
legend('simulation: norme de la position finale',fit)
grid on
% set(gca, 'YScale', 'log')
hold off
...
set(gca,'fontsize',fs)
xlabel('$\Delta$t [s]')
ylabel('$|\Delta\vec{x}(t_{fin})|$ [m]')
if p1(2)>0
   fit = sprintf('fit:$y=$%0.5g$x+$%0.5g',p1(1),p1(2));
else 
   fit = sprintf('fit:$y=$%0.5g$x$%0.5g',p1(1),p1(2));
end
legend('erreur sur la position finale',fit)
grid on
hold off
