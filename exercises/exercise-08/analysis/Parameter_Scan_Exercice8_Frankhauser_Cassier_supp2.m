...
input      = 'configuration_supp1.in'; % Nom du fichier d'entrée 

close all;
sigma_norm=[0.032748655876143];
...
set(gca,'fontsize',fs)
xlabel('$t$ ')
ylabel('$\langle \Delta x\rangle (t)$ ')
legend(titre,'Location','north','NumColumns',4);
grid on
title(legend, '$\sigma_{norm}$');
hold off
...
set(gca,'fontsize',fs)
xlabel('$t$ ')
ylabel('$\langle \Delta p\rangle (t)$ ')
legend(titre,'Location','north','NumColumns',4);
grid on
title(legend, '$\sigma_{norm}$');
hold off
