...
eval(sprintf('!Exercice8_2023_student_supp.exe'))
%%
fl =16;
...
%     end
%     if(T1<=t(i) && t(i) <T2+T1)
%       xana(i)=B*sin(w2*t(i)+w1*T1);
%       pana(i)=B*w2*cos(w2*t(i)+w1*T1);
%     end
% end
% %%
% % delta =0;
...
xlabel('$t$ [s]')
ylabel('$\psi$')
legend('$\langle x\rangle $')
grid on

figure
plot(t,pmoy);%,t,x2moy,t,pmoy,t,p2moy
xlabel('$t$ [s]')
ylabel('$\psi$')
legend('$\langle p\rangle $')
grid on
