...
main(int argc, char** argv)
{
  ...
  complex<double> a = 1i*hbar*dt/(4*m*dx*dx);; // Coefficient complexe a de l'equation (4.100), à modifier par l'élève
    ...
    {
        complex<double> b = 1i*dt*V(x[i],m,delta,w1,w2)/(hbar*2);; // Coefficient complexe b de l'equation (4.100), à modifier par l'élève
    // TODO: éléments de matrices diagonales
        dH[i] = pow(hbar,2)/(m*dx*dx)+V(x[i],m,delta,w1,w2);
        dA[i] = 1.+2.*a+b;
        dB[i] = 1.-2.*a-b;
    }
    ...
    cout << "V(xc)= " << V(x_local_max, m, delta, w1, w2) << endl;
