		...
		vector<double> &beta2, string &bc_l, string &bc_r, int &N)
{
      if (bc_l == "fixe"){
        ...
        fnext[0] =  A*sin(omega*(t+dt)); // TODO : Completer la condition au bord gauche harmonique
      }else if (bc_l =="sortie"){
        fnext[0] = fnow[0]+sqrt(abs(beta2[0]))*(fnow[0]-fnow[1]); // TODO : Completer la condition au bord gauche "sortie de l'onde"
      }else{
      ...
      if (bc_r == "fixe"){
        ...
        fnext[N-1] =  A*sin(omega*(t+dt)); // TODO : Completer la condition au bord gauche harmonique
      }else if(bc_r == "libre"){
        fnext[N-1] = fnext[N-2]; // TODO : Completer la condition au bord droit harmonique
      }else if (bc_r =="sortie"){
        fnext[N-1] = fnow[N-1]-sqrt(abs(beta2[N-1]))*(fnow[N-1]-fnow[N-2]); // TODO : Completer la condition au bord droit "sortie de l'onde"
      }else{
...
double energie(vector<double> const& f, double const& dx)
{
  ...
  for(unsigned int i(0); i<f.size()-1; ++i)
    energie_ += dx*(f[i]*f[i]+f[i+1]*f[i+1])/2.;
...
int main(int argc, char* argv[])
{
  ...
  string inputPath("input"); // Fichier d'input par defaut
  ...
  vector<double> x(N) ;
  vector<double> fpast(N), fnow(N), fnext(N), beta2(N), fnowth(N);
  ...
  cout << "dt is "<< dt<<endl; cout << "dx is " <<dx<< endl;
  ...
  ofstream fichier_ana((output + "_ana").c_str());
  fichier_ana.precision(15);
  ...
  for(int i(0); i<N; ++i)
  {
    fpast[i] = 0.;
    fnow[i]  = 0.;
    beta2[i] = vel2[i]*dt*dt*(1.0/(dx*dx));
    if(initialization=="cos"){
	fnow[i] = fmn*sin(x[i]*minit*PI/x[N-1])*sin(minit*PI*sqrt(vel2[i])/x[N-1]);
  ...
  for(t=0.; t<tfin-.5*dt; t+=dt)
  {
    ...
    for(int i(1); i<N-1; ++i)
    {
      ...
      if(schema == "B"){
        fnext[i] += fnow[i+1]*((beta2[i+1]-beta2[i-1])/4)-0.25*fnow[i-1]*(beta2[i+1]-beta2[i-1]); // TODO : Compléter le schéma B
      }else if(schema == "C"){
        fnext[i] += beta2[i+1]*(0.5*(fnow[i+1]-fnow[i-1])+fnow[i])+beta2[i-1]*(-0.5*(fnow[i+1]-fnow[i-1])+fnow[i])-2*beta2[i]*fnow[i]; // TODO : Compléter le schéma C
      }
    }
    ...
    if (lance)
    {
    	if (t<=((2.*PI)/omega) )
    	{
    		...
    		boundary_condition(fnext, fnow, A, omega, t, dt, beta2, aha, bc_r, N);
    	}else{
    		string aha="fixe";
    		boundary_condition(fnext, fnow, A, omega, t, dt, beta2, aha, bc_r, N);
    	}
  ...
  for(t=0.; t<tfin; t+=dt)
  {
	  	for(int i(0); i<N; ++i)
	  {
	  	fnowth[i] = fmn*sin(x[i]*minit*PI/x[N-1])*sin(minit*PI*sqrt(vel2[i])*(1.0+t)/x[N-1]);
	    if(initialization=="cos"){
		fnowth[i] = fmn*sin(x[i]*minit*PI/x[N-1])*sin(minit*PI*sqrt(vel2[i])*(1.0+t)/x[N-1]);
	    }else if(initialization=="sin"){
    	fnowth[i] = 0;
